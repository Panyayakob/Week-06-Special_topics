...
Docker: สะดวกตั้งค่าสิ่งแวดล้อมได้เร็ว ไม่ต้องกังวล dependency conflicts, พกพาง่าย, แยกสภาพแวดล้อมชัดเจน

Native: ประสิทธิภาพสูงกว่า (ไม่ต้องรันผ่าน container), เข้าถึงระบบไฟล์และฮาร์ดแวร์ได้โดยตรง
2. **Build Process**: อธิบายขั้นตอนการ build ของ ESP-IDF ใน Docker container ตั้งแต่ source code จนได้ binary
Docker รัน container ที่มี ESP-IDF เตรียมไว้

คัดลอก source code เข้า container

ใช้ CMake กับ Ninja คอมไพล์โค้ดใน container

สร้าง binary (.bin, .elf) และไฟล์แฟลชต่างๆ
3. **CMake Files**: บทบาทของไฟล์ CMakeLists.txt แต่ละไฟล์คืออะไร และทำงานอย่างไรใน Docker environment?
CMakeLists.txt ในโปรเจกต์หลัก: กำหนด source, target, และเรียกใช้ component

CMakeLists.txt ในแต่ละ component: กำหนดวิธี build และ dependencies ของ component นั้น

ใน Docker environment ทำงานเหมือน native เพราะ container คือ environment ที่จำลองระบบ Linux เต็มรูปแบบ
4. **Git Ignore**: ไฟล์ .gitignore มีความสำคัญอย่างไรสำหรับ ESP32 project development?
ป้องกันไม่ให้ไฟล์ build, ไฟล์ชั่วคราว หรือโฟลเดอร์ build/ ถูก push ขึ้น Git

ช่วยลดขนาด repo และป้องกันปัญหา merge conflict จากไฟล์ที่สร้างอัตโนมัติ
5. **Container Persistence**: ข้อมูลใดบ้างที่จะหายไปเมื่อ restart container และข้อมูลใดที่จะอยู่ต่อ?
ข้อมูลใน container เช่นไฟล์ใน /project ที่ mount volume จะอยู่ต่อ

ข้อมูลภายใน container ที่ไม่ได้ mount volume จะหายเมื่อ container ถูกลบหรือ restart ใหม่
6. **Development Workflow**: เปรียบเทียบ workflow การพัฒนาระหว่างการใช้ Docker กับการทำงานบน native system
Docker: สภาพแวดล้อมเสถียร, setup เร็ว, เหมาะกับทีมและ CI/CD

Native: เข้าถึงเครื่องได้เต็มที่, ติดตั้งครั้งเดียว, ปรับแต่งง่าย แต่ setup ซับซ้อนและเสี่ยง dependency conflict
